...
class TestTShirtOrchestrator:
    ...
    def sample_product(self):
        """Create a sample ProdigiProduct."""
        return ProdigiProduct(
            ...
            retail_price=15.0,
            currency="USD",
            product_url="https://dashboard.prodigi.com/orders/ord_12345",
            status="InProgress",
        )
    ...
    async def test_initialize(self, orchestrator):
        ...
        with patch.object(orchestrator.prodigi_client, 'initialize', new_callable=AsyncMock) as mock_init:
    ...
    async def test_cleanup(self, orchestrator):
        ...
        with patch.object(orchestrator.prodigi_client, 'cleanup', new_callable=AsyncMock) as mock_cleanup:
    ...
    async def test_process_tshirt_request_success(
        ...
        assert "ord_12345" in result.product_url
    ...
    @pytest.mark.asyncio
    async def test_process_tshirt_request_prodigi_failure(
        self,
        orchestrator,
        sample_request,
    ):
        """Test request processing when Prodigi API fails."""
        with patch.object(
            orchestrator.llm_parser,
            'parse_message',
            new_callable=AsyncMock,
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    'create_product',
                    new_callable=AsyncMock,
                    side_effect=Exception("Prodigi API error"),
                ):
                    result = await orchestrator.process_tshirt_request(
                        message="test message",
                        user_id="test_user",
        ...
        assert result.success is False
        assert "Prodigi API error" in result.error_message
        assert result.product_url is None
